...
#define As(stage,li,lj) As[stage][(li) + (lj)*MS]
#define Bs(stage,li,lj) Bs[stage][(li) + (lj)*MS]
...
void sgemm_kernel_6(const float* A, const float* B, float* C, int m, int n, int k) 
{
    ...
    int warp_id = tIdx / 32;
    int lane_id = tIdx % 32;
    int warp_i = warp_id % 4, warp_j = warp_id / 4;
    int lane_i = lane_id % 4, lane_j = lane_id / 4;
    int li_c = warp_i * 32 + lane_i * 8;
    int lj_c = warp_j * 64 + lane_j * 8;

    // shared-memory buffers (double buffering)
    __shared__ float As[2][MS * KS];   // 2 x 128 x 8 = 2 x 1024 elements of A
    __shared__ float Bs[2][KS * NS];   // 2 x 8 x 128 = 2 x 1024 elements of B
    int stage = 0; 
    ...
    // pre-load first tile (p = 0) into As and Bs
    VLOAD(((float4 *)As[stage])[tIdx], &A(bi + li_a, lj_a));
    ...
    VLOAD(Br1, &B(li_b, bj + lj_b));
    Bs(stage, lj_b, li_b    ) = Br1.x;
    Bs(stage, lj_b, li_b + 1) = Br1.y;
    Bs(stage, lj_b, li_b + 2) = Br1.z;
    Bs(stage, lj_b, li_b + 3) = Br1.w;
    ...
    for (int p = 0; p < k; p += KS) {
        ...
        if (p + KS < k) {
            ...
            VLOAD(((float4 *)As[next_stage])[tIdx], &A(bi + li_a, p + KS + lj_a));

            // thread loads four elements from a block of B (8 x 128) and stores in Bs in transposed 
            // form, i.e. (128 x 8). Transposed form allows vectorized access to rows of B.      
            VLOAD(Br1, &B(p + KS + li_b, bj + lj_b));
            Bs(next_stage, lj_b, li_b    ) = Br1.x;
            Bs(next_stage, lj_b, li_b + 1) = Br1.y;
            Bs(next_stage, lj_b, li_b + 2) = Br1.z;
            Bs(next_stage, lj_b, li_b + 3) = Br1.w;
        }

        #pragma unroll
        for (int lp = 0; lp < KS; lp++) {
            // Load one column (8x1) of As to registers
            VLOAD(Ar1, &As(stage, li_c    , lp))
            VLOAD(Ar2, &As(stage, li_c + 4, lp))
            // Load one row (1x8) of Bs to registers (Bs is stored in transposed form!)
            VLOAD(Br1, &Bs(stage, lj_c    , lp))
            VLOAD(Br2, &Bs(stage, lj_c + 4, lp))
